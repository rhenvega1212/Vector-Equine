 ...
 * Reads the live stage/rollout from the DB plus the viewer's per-user overrides,
 * then evaluates each flag. Falls back to the registry default stage if a flag
 * row is missing, and to "all off" on any error.
 *
 * Pass the *effective* profile (respecting impersonation) so flags reflect who
 * the viewer is acting as.
 */
export async function getFlagsForProfile(
  ...
  try {
    ...
    const overrideByKey = new Map<string, boolean>(
      ((overrideRows ?? []) as { flag_key: string; enabled: boolean }[]).map(
        (r) => [r.flag_key, r.enabled]
      )
    );
