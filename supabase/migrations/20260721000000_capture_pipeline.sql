-- Brief 11: Capture Live pipeline (join codes, transcript, media stubs)
...
-- Guest trainer writes go through service-role API routes (join token), not RLS.
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'update_updated_at_column') THEN
    ...
    DROP TRIGGER IF EXISTS update_session_media_assets_updated_at ON session_media_assets;
    CREATE TRIGGER update_session_media_assets_updated_at
      BEFORE UPDATE ON session_media_assets
      FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
  END IF;
END $$;
